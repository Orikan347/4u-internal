...
os.environ.setdefault("LINE_FUNCTIONAL_TEST_CHANNEL", "1")
...
def functional_runtime_dir():
    """Return the source directory or the folder beside a frozen test EXE."""
...
def get_config_dir():
    """取得設定檔目錄"""
    if FUNCTIONAL_TEST_CHANNEL:
        test_dir = Path(os.environ.get(
            'LINE_FUNCTIONAL_TEST_DATA_DIR',
            str(functional_runtime_dir() / 'functional_test_data')
        ))
...
class LicenseManager:
    ...
    def __init__(self):
        self.data_dir = (
            get_config_dir()
            if FUNCTIONAL_TEST_CHANNEL
            else Path(os.environ.get('APPDATA', Path.home())) / 'LINE自動發訊息'
        )
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self.license_file = (
            self.data_dir / 'functional_test_license.json'
            if FUNCTIONAL_TEST_CHANNEL
            else self.data_dir / 'license.json'
        )
        if FUNCTIONAL_TEST_CHANNEL and not self.license_file.exists():
            seed_file = functional_runtime_dir() / 'functional_test_license.json'
            try:
                if seed_file.exists():
                    self.license_file.write_text(seed_file.read_text(encoding='utf-8'), encoding='utf-8')
            except Exception:
                pass
    ...
    def is_licensed(self):
        ...
        if (not FUNCTIONAL_TEST_CHANNEL and
                str(self.data.get('channel', '')).strip().lower() in ('functional-test', 'test')):
            return False
    ...
    def refresh_lease(self):
        """啟動時重新驗證 lease；明確拒絕不可由本機快取覆蓋。"""
        if not self.is_licensed():
            return False
        if FUNCTIONAL_TEST_CHANNEL:
            return self._cache_is_usable()
        client = self._get_license_client()
        if not client:
            # 沒有端點不是網路錯誤；不可把本機檔案當成線上驗證。
            return False
        try:
            result = client.verify_lease(
                self.data.get('lease_token', ''),
                PRODUCT_ID,
                self.get_device_id(),
            )
        except Exception:
            # 網路錯誤可暫時沿用短效 cache；明確 API 拒絕則不能走這裡的覆蓋。
            return self._cache_is_usable()
    ...
    def activate_license(self, key, email=""):
        """
        啟用授權碼 — 必須完成線上驗證；離線不得啟用。
        """
        key = key.strip().upper()
        if not key:
            return False, "請輸入授權碼"
        ...
        if client:
            try:
                result = client.issue_lease(
                    email, key, PRODUCT_ID, self.get_device_id(), APP_VERSION, "windows"
                )
                if result.get('status') == 'allowed' and result.get('lease_token') and result.get('lease_expires_at'):
                    expire = result.get('lease_expires_at', '')
                    ...
                    self.data['lease_token'] = result['lease_token']
                    self.data['lease_expires_at'] = expire
                    self.data['verified_online'] = True
                    self.data['verified_at'] = datetime.now().astimezone().isoformat()
                    self.data['verify_date'] = datetime.now().isoformat()
                    ...
                    msg = f"訂閱授權啟用成功！短效憑證至：{expire}"
                    if customer:
                        msg += f"\n歡迎，{customer}！"
                    return True, msg
                else:
                    reason = result.get('status', 'denied')
                    messages = {
                        'expired': '此授權碼已過期，請聯繫續約。',
                        'suspended': '此授權已停用，請聯繫管理者。',
                        'product_not_in_plan': '目前方案不包含 LINE 工具。',
                        'device_limit_reached': '此訂閱已達裝置上限。',
                    }
                    return False, messages.get(reason, '授權碼驗證失敗，請確認後重試。')
            except Exception:
                return False, "無法連線驗證，請確認網路後重試。"

        return False, "無法連線驗證，未完成線上驗證前不能使用。"
...
class AuthorizationRequiredWindow:
    def __init__(self, license_mgr):
        ...
        tk.Label(body, text="尚未完成授權驗證",
                 font=("Microsoft JhengHei", 20, "bold"),
                 fg=C_RED, bg=C_BG).pack(pady=(10, 5))

        tk.Label(body, text="LINE 自動發訊息需要有效授權。\n請輸入線上驗證可通過的授權碼。",
        ...
        tk.Label(body, text="Email：",
                 font=("Microsoft JhengHei", 10),
                 fg=C_WHITE, bg=C_BG, anchor='w').pack(fill='x')
        self.email_entry = tk.Entry(body, font=("Arial", 11),
                                     bg=C_BG_DARK, fg=C_WHITE,
                                     insertbackground=C_WHITE, relief='flat')
        self.email_entry.pack(fill='x', pady=(3, 10), ipady=6)
    ...
    def _activate(self):
        ...
        success, msg = self.license_mgr.activate_license(key, email)
    ...
    def _submit(self):
        ...
        try:
            local_file = os.path.join(
                str(get_config_dir()), 'survey_responses.json'
            )
            responses = []
...
if __name__ == '__main__':
    if (not FUNCTIONAL_TEST_CHANNEL and
            os.environ.get('DEAL_ALLIANCE_FUNCTIONAL_TEST', '').strip() == '1'):
        root = tk.Tk(); root.withdraw()
        messagebox.showerror(
            "版本通道不相容",
            "正式候選版拒絕功能測試 channel，請使用獨立的 Windows functional-test 版本。"
        )
        sys.exit(0)
