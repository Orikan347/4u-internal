...
def main() -> int:
    ...
    if not RELEASE_ID_RE.fullmatch(args.release_id):
        raise SystemExit("WIN-EVIDENCE-001: valid allocated release_id is required")

    required_inputs = (
        args.allocation, args.binding_manifest, args.bound_source, args.helper, args.exe, args.self_test_report,
    )
    ...
    sbom = {
        ...
        "metadata": {
            "component": {
                ...
                "version": version,
                "properties": [
                    {"name": "deal_alliance.release_id", "value": release_id},
